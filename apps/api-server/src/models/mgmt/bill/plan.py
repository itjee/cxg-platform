...
class Plan(BaseModel):
    ...
    start_time = Column(
        TIMESTAMP(timezone=False), nullable=False, comment="요금제 시작일"
    )
    close_time = Column(
        TIMESTAMP(timezone=False), nullable=True, comment="요금제 종료일"
    )
    status = Column(
        String(20),
        nullable=False,
        default="ACTIVE",
        comment="요금제 상태(ACTIVE/INACTIVE)",
    )
    deleted = Column(
    ...
    def __repr__(self):
        return (
            ...
            f"price={self.base_price} status={self.status}>"
        )
