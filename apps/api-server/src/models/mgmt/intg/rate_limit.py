...
class RateLimit(BaseModel):
    ...
    expires_at = Column(
        TIMESTAMP(timezone=True), comment="제한 규칙 만료 시각"
    )
    deleted = Column(
    ...
    user = relationship("TenantUser", back_populates="rate_limits")
    api_key = relationship("TenantApiKey", back_populates="rate_limits")

    def __repr__(self):
        return f"<RateLimit type={self.limit_type} value={self.limit_value}>"
