...
class TenantStat(BaseModel):
    ...
    uptime_rate = Column(Numeric(5, 2), default=100, comment="가동률(%)")
    feature_adoption_rate = Column(
        Numeric(5, 2), default=0, comment="기능 채택률(%)"
    )
    user_satisfaction_score = Column(
        Numeric(3, 1), comment="사용자 만족도 점수(1~5)"
    )
    status = Column(
        String(20),
        nullable=False,
    ...
    # 관계
    tenant = relationship("Tenant", back_populates="tenant_stats")

    def __repr__(self):
        return (
            f"<TenantStat tenant={self.tenant_id} "
